...
import com.huiyouhui.lib.MyRecycleView.AnimImageView;
import com.huiyouhui.lib.MyRecycleView.AnimView;
import com.huiyouhui.lib.MyRecycleView.MyGridLayoutManager;
import com.huiyouhui.lib.MyRecycleView.MyLinearLayoutManager;
import com.huiyouhui.lib.MyRecycleView.MyStaggeredGridLayoutManager;
import com.huiyouhui.lib.MyRecycleView.OverScrollListener;
 ...
 */
public final class MyRecycleView extends RecyclerView implements Runnable {

    private Context mContext;

    private static MyRecycleView rfView;
    ...
    public void setHeaderImage(ImageView headerImage) {
        ...
        headerImageHeight = MyRecycleView.this.headerImage.getHeight();
        // 防止第一次拉伸的时候headerImage.getLayoutParams().height = 0
        if (headerImageHeight <= 0) {
            headerImageHeight = MyRecycleView.this.headerImage.getLayoutParams().height;
        } else {
    ...
    public void run() {
        ...
        if (manager instanceof MyLinearLayoutManager) {
            // ListView布局
            ((MyLinearLayoutManager) manager).setOverScrollListener(mOverScrollListener);
        } else if (manager instanceof MyGridLayoutManager) {
            layoutGridAttach((MyGridLayoutManager) manager);
        } else if (manager instanceof MyStaggeredGridLayoutManager) {
            layoutStaggeredGridHeadAttach((MyStaggeredGridLayoutManager) manager);
        }
     ...
     *
     * @param manager {@link MyStaggeredGridLayoutManager}
     */
    private void layoutStaggeredGridHeadAttach(MyStaggeredGridLayoutManager manager) {
    ...
    /**
     * 给{@link MyGridLayoutManager}附加头部脚部和滑动过度监听
     *
     * @param manager {@link MyGridLayoutManager}
     */
    private void layoutGridAttach(final MyGridLayoutManager manager) {
        // GridView布局
        manager.setOverScrollListener(mOverScrollListener);
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadDataListener != null && !isLoadingData) {
            ...
            if (layoutManager instanceof MyGridLayoutManager) {
                lastVisibleItemPosition = ((MyGridLayoutManager) layoutManager).findLastVisibleItemPosition();
            } else if (layoutManager instanceof MyStaggeredGridLayoutManager) {
                int[] into = new int[((MyStaggeredGridLayoutManager) layoutManager).getSpanCount()];
                ((MyStaggeredGridLayoutManager) layoutManager).findLastVisibleItemPositions(into);
                lastVisibleItemPosition = findMax(into);
            } else {
                lastVisibleItemPosition = ((MyLinearLayoutManager) layoutManager).findLastVisibleItemPosition();
            }
